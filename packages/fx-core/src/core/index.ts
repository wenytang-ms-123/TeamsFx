...
export class FxCore implements Core {
  ...
  async createProject(inputs: Inputs, ctx?: CoreHookContext): Promise<Result<string, FxError>> {
    ...
    if (scratch === ScratchOptionNo.id) {
      ...
      const solutionContext: SolutionContext = {
        ...
        answers: inputs,
        encryptionProvider: new Encryption(projectSettings.projectId),
      };
    ...
    if (sample && sample.data && folder) {
      ...
      try {
        ...
        if (fetchRes !== undefined) {
          ...
          sendTelemetryEvent(this.tools.telemetryReporter, inputs, TelemetryEvent.DownloadSample, {
  ...
  async encrypt(
    ...
    return ok(ctx!.solutionContext!.encryptionProvider!.encrypt(secret));
  }
  ...
  async decrypt(
    ...
    return ok(ctx!.solutionContext!.encryptionProvider!.decrypt(secret));
  }
