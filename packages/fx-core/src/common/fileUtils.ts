...
export async function readJson(filePath: string): Promise<any> {
  if(!await fs.pathExists(filePath)) {
    throw new UserError({source: CoreSource, name: "FileNotFoundError", message: `File not found, make sure you don't move the original file: ${filePath}`});
  }
  let rawError;
  for (let i = 0; i < 5; ++i) {
    try {
   ...
   */
  const fxError: SystemError = new SystemError({name: "ReadJsonError", error: rawError as Error, source: CoreSource});
